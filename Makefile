...
PACKAGES = \
	tezos-node cpdf conf-pkg-config conf-zlib bigstringaf decompress camlzip menhirLib \
...
ocaml-versions/%.bench: check_url depend log_sandmark_hash ocaml-versions/%.json _opam/% .FORCE
	...
	opam list --rec --resolve tezos-node
	@{ echo '(lang dune 1.0)'; \
	...
	@{ if [ "$(BUILD_ONLY)" -eq 0 ]; then												\
		...
		IS_MACRO_PLUS=`jq -r '. | .benchmarks | .[] | select(.tags | index("macro_plus_bench") != null)' $(RUN_CONFIG_JSON)`;   \
		...
		if [ "$$IS_MACRO_PLUS" ]; then												\
		  echo "$$IS_MACRO_PLUS checked";											\
		else															\
		  echo "$$IS_MACRO_PLUS not checked";											\
		fi;															\
...
%_macro_plus.json: %.json
	jq '{wrappers : .wrappers, benchmarks: [.benchmarks | .[] | select(.tags | index("macro_plus_bench") != null)]}' < $< > $@
